...
// ============================================================================
// Type Definitions (matching coordinator)
// ============================================================================
...
class FileBasedAgent {
  ...
  private currentTask: TaskMessage | null = null;
   ...
   * Check for new instructions
   */
  private async checkForInstructions(): Promise<void> {
   ...
   */
  private async performTaskLogic(task: TaskMessage): Promise<any> {
  ...
  private async performAnalysis(input: any): Promise<any> {
    ...
    const mean = data.reduce((a: number, b: number) => a + b, 0) / data.length || 0;
    const sorted = [...data].sort((a, b) => a - b);
    const median = sorted[Math.floor(sorted.length / 2)] || 0;
    const variance = data.reduce((sum: number, x: number) => sum + Math.pow(x - mean, 2), 0) / data.length || 0;
   ...
   */
  private async performGenericTask(input: any): Promise<any> {
   ...
   */
  private validateOutput(output: any, validation: any): void {
   ...
   * Update agent status
   */
  private async updateAgentStatus(status: 'AVAILABLE' | 'BUSY' | 'OFFLINE'): Promise<void> {
    const registration = {
      agentId: this.agentId,
      capabilities: this.capabilities,
      status,
