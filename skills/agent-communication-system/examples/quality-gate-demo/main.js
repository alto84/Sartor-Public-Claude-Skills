/**
 * Quality Gate Demo (JavaScript version)
...
class QualityGateSystem {
   ...
   */
  async evaluateAll(output, attempt = 1) {
   ...
   */
  async evaluateGate(gate, output) {
    switch (gate.type) {
      case 'CITATION_CHECK':
   ...
   * Citation validation
   */
  checkCitations(criteria, output) {
   ...
   */
  checkConfidence(criteria, output) {
   ...
   */
  async conductPeerReview(criteria, output) {
...
class ResearchAgent {
   ...
   */
  async reviseOutput(output, feedback) {
...
async function runScenario(scenario = 'pass') {
...
// Export for testing
module.exports = { QualityGateSystem, ResearchAgent };